...
import br.com.fenix.apiintegracao.service.ServiceJdbcItemFull
...
abstract class ControllerJdbcBaseItemFull<ID, E : EntityBase<ID, E>, D : DtoBase<ID>, C : ControllerJdbcBaseItemFull<ID, E, D, C>>(
    repository: RepositoryJdbcItemFull<E, ID>,
    factory: EntityFactory<ID, E>
) {
    protected val service: ServiceJdbcItemFull<ID, E, D, C>
    ...
    init {
        ...
        service = object : ServiceJdbcItemFull<ID, E, D, C>(repository, factory, clazzEntity, clazzDto, clazzController) {
...
* Retorna uma lista contendo os objetos-filho completos após a atualização, na mesma ordem em que foram enviados na requisição (status `200 OK`).""",
        operationId = "updateListBaseFull"
    )
    @JsonView(Views.Summary::class)
    @PutMapping(
        "$TABLES_URL/{idParent}/lista",
...
* A ordem dos IDs na lista de resposta corresponde à ordem dos objetos enviados na requisição.""",
    ...
    @Operation(
        ...
        description = """Deleta um único registro-filho específico da tabela informada com base no seu identificador único (ID).

**Parâmetros de Path:**
* **`table`**: O nome da tabela de onde o registro será deletado.
* **`id`**: O ID do registro-filho a ser deletado.
...
* Retorna uma mensagem de confirmação (ex: "Ok") ou um status `204 No Content` em caso de sucesso."""
    ...
    @Operation(
        ...
        description = """Deleta múltiplos registros-filho em uma única operação em lote na tabela especificada, com base em uma lista de IDs.

**Parâemons de Path:**
* **`table`**: O nome da tabela de onde os registros serão deletados.
                        
**Corpo da Requisição:**
* Aceita um array JSON contendo os `IDs` dos registros-filho a serem deletados.
                        
**Corpo da Resposta:**
* Retorna uma mensagem de confirmação (ex: "Ok") ou um status `204 No Content` em caso de sucesso.""",
        operationId = "deleteListIdBaseFull"
    )
    @DeleteMapping(
        "$TABLES_URL/lista",
        consumes = arrayOf(
