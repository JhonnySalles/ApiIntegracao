...
data class KanjiInfo(
    ...
    companion object : EntityFactory<UUID?, KanjiInfo> {
        override fun create(id: UUID?): KanjiInfo = KanjiInfo(id, 0, "", "", 0, "")
    }
    ...
    override fun patch(source: KanjiInfo) {
        ...
        if (source.word.isNotEmpty())
            this.word = source.word

        if (source.readInfo.isNotEmpty())
            this.readInfo = source.readInfo

        if (source.frequency > 0)
            this.frequency = source.frequency

        if (source.tabela.isNotEmpty())
            this.tabela = source.tabela
    }
